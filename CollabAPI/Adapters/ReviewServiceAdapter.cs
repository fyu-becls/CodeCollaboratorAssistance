...
namespace CollabAPI
{
    public class ReviewServiceAdapter : BaseServiceAdapter, IReviewService, IBaseService
    {
        ...
        public Review.ReviewsFindResponse GetReviewsByCustomField(string field, string text)
        {
            ReviewService.findReviewsByCustomFieldValue findReviewsByCustomField = new ReviewService.findReviewsByCustomFieldValue();
            findReviewsByCustomField.customFieldName = field;
            findReviewsByCustomField.searchText = text;

            JsonResult jsonResult = this.SendRequest((object)findReviewsByCustomField, true);
            Review.ReviewsFindResponse findResponse = null;
            if (jsonResult != null)
            {
                findResponse = jsonResult.GetResponse<Review.ReviewsFindResponse>();
            }

            return findResponse;
        }

        public Review.ReviewsFindResponse GetReviews(string field, string text, int findPlace)
        {

            ReviewService.findReviews findReviews = new ReviewService.findReviews();
            findReviews.customFieldName = field;
            findReviews.searchText = text;
            findReviews.findPlace = findPlace;

            JsonResult jsonResult = this.SendRequest((object)findReviews, true);
            Review.ReviewsFindResponse findResponse = null;
            if (jsonResult != null)
            {
                findResponse = jsonResult.GetResponse<Review.ReviewsFindResponse>();
